...
from utils.storage import load_table
from utils.datas import get_today_date_str, format_date_br, calcular_status_alerta_horario, parse_date
...
def render_dashboard(set_page_callback=None):
        ...
        <div style="background: linear-gradient(135deg, #24398e 0%, #1e293b 100%); padding: 18px 24px; border-radius: 16px; color: white; margin-bottom: 24px; box-shadow: 0 4px 15px rgba(0,0,0,0.08);">
            <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap;">
                <div>
                    ...
                    <p style="margin: 4px 0 0 0; color: #cbd5e1; font-size: 14px;">Visão em tempo real da agenda, serviços de banho e tosa, hotelzinho e fluxo de caixa.</p>
                </div>
    ...
    with col3:
        st.markdown(f"""
            <div class="kpi-card" style="border-left: 5px solid #10b981;">
                <div style="font-size: 13px; color: #64748b; font-weight: 600; text-transform: uppercase;">💰 Faturamento (Mês)</div>
                <div style="font-size: 24px; font-weight: 800; color: #10b981; margin: 4px 0;">{formatar_moeda(metricas_mes['total_entradas'])}</div>
                <div style="font-size: 12px; color: #64748b;">Receita bruta total</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        st.markdown(f"""
            <div class="kpi-card" style="border-left: 5px solid #d82678;">
                ...
                <div style="font-size: 12px; color: #64748b;">Entradas - Saídas - Desc.</div>
            </div>
        """, unsafe_allow_html=True)

    st.markdown("<div style='height: 15px;'></div>", unsafe_allow_html=True)

    # ==================== SEÇÃO DE ALERTAS DE HORÁRIO E EVENTOS DO DIA ====================
    alertas_atendimento = []
    ...
    if alertas_atendimento or saidas_hoje:
        st.markdown("### 🔔 Central de Alertas e Notificações de Hoje")
